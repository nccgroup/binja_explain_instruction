...
def preprocess_LLIL_IF(bv, llil_instruction):
    ...
    # We have to use the lifted IL since the LLIL ignores comparisons and tests
    lifted_instruction = list(
        [k for k in find_lifted_il(func, llil_instruction.address) if k.operation == LowLevelILOperation.LLIL_IF]
    )[0]
    lifted_il = func.lifted_il
...
def preprocess_LLIL_FLAG(bv, llil_instruction):
    ...
    if llil_instruction.src.temp:
        ...
        src = llil_instruction.function.get_ssa_flag_definition(flag)
        if hasattr(src, "src"):
            ...
            lifted_instruction = list(
                [
                    k
                    for k in find_lifted_il(llil_instruction.function.source_function, llil_instruction.address)
        ...
        if hasattr(llil_instruction, attr):  # Not all instructions will have the same attributes
            ...
            if (
                ...
                llil_instruction.__setattr__(
                    attr,
                    ("{}" if unexplained.operation in no_paren else "({})").format(explain_llil(bv, unexplained)),
                )
    return llil_instruction
